...
procedure Day_22_part_2 is
    ...
    type Long_AF is range -(2 ** 63) .. +(2 ** 63 - 1);
    type Operation is record
        A : Long_AF;
        B : Long_AF;
    end record;
    M : Long_AF := 119315717514047; -- assuming M is prime
    K : Long_AF := 101741582076661;
    F : Operation;
    ...
    function Pow_Mod(Input_X: Long_AF; Input_N:Long_AF; M:Long_AF) return Long_AF is
        Y : Long_AF := 1;
        N : Long_AF := Input_N;
        X : Long_AF := Input_X;
    begin
    ...
    function Inverse_Mod(A:Long_AF; M: Long_AF) return Long_AF is
    begin
        return Pow_Mod(A, M - 2, M);
    end Inverse_Mod;
    function F_Repeated_K_Times_Inverse(F: Operation; K: Long_AF; X: Long_AF) return Long_AF is
        A : Long_AF := Pow_Mod(F.A, K, M);
        B : Long_AF := Mul_Mod(Mul_Mod(F.B, (1 - A), M), Inverse_Mod(1 - F.A, M), M);
    begin
...
begin
    begin
      ...
      Put_Line(Long_AF'Image(F_Repeated_K_Times_Inverse(F, K, 2020)));
    end;
end Day_22_part_2;
